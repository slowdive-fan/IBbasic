...
namespace Birthday.iOS
{
    public class SaveAndLoad_iOS : ISaveAndLoad
    {
        ...
        public string LoadStringFromUserFolder(string fullPath)
        {
            ...
            Stream stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets" + ConvertFullPath(fullPath, "."));
            if (stream != null)
            {
        ...
        public string LoadStringFromAssetFolder(string fullPath)
        {
            ...
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS." + filename);
            }
            if (stream != null)
            {
                using (var reader = new System.IO.StreamReader(stream))
        ...
        public string LoadStringFromEitherFolder(string assetFolderpath, string userFolderpath)
        {
            ...
            //check in Assests folder last
            Assembly assembly = GetType().GetTypeInfo().Assembly;
            Stream stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets" + ConvertFullPath(assetFolderpath, "."));
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS." + filename);
            }
        ...
        {            
            ...
            Assembly assembly = GetType().GetTypeInfo().Assembly;
            Stream stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.modules." + modFilenameNoExtension + "." + modFilename);
            if (stream != null)
            {
                using (var reader = new System.IO.StreamReader(stream))
                {
        ...
        {            
            Assembly assembly = GetType().GetTypeInfo().Assembly;
            Stream stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.modules." + mdl.moduleName + ".graphics." + filename);            
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.modules." + mdl.moduleName + ".graphics." + filename + ".png");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS." + filename);
            ...
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS." + filename + ".png");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS." + filename + ".jpg");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.graphics." + filename);
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.graphics." + filename + ".png");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.graphics." + filename + ".jpg");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.tiles." + filename);
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.tiles." + filename + ".png");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.tiles." + filename + ".jpg");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.ui." + filename);
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.ui." + filename + ".png");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.ui." + filename + ".jpg");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.graphics.ui_missingtexture.png");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.ui_missingtexture.png");
            }
        ...
        public List<string> GetAllFilesWithExtensionFromBothFolders(string assetFolderpath, string userFolderpath, string extension)
        {
            ...
            foreach (var res in assembly.GetManifestResourceNames())
            {
                if ((res.Contains("Birthday.iOS.Assets" + ConvertFullPath(userFolderpath, "."))) && (res.EndsWith(extension)))
                {
                    string[] split = res.Split('.');
                    list.Add(split[split.Length - 2]);
                }
            ...
            foreach (var res in assembly.GetManifestResourceNames())
            {
                if ((res.Contains("Birthday.iOS.Assets" + ConvertFullPath(assetFolderpath, "."))) && (res.EndsWith(extension)))
                {
                    string[] split = res.Split('.');
                    list.Add(split[split.Length - 2]);
                }
        ...
        {
            Assembly assembly = GetType().GetTypeInfo().Assembly;
            var stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.modules." + gv.mod.moduleName + "." + filename);
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.modules." + gv.mod.moduleName + "." + filename + ".wav");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.modules." + gv.mod.moduleName + "." + filename + ".mp3");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.sounds." + filename);
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.sounds." + filename + ".wav");
            }
            if (stream == null)
            {
                stream = assembly.GetManifestResourceStream("Birthday.iOS.Assets.sounds." + filename + ".mp3");
            }
            return stream;
        }
