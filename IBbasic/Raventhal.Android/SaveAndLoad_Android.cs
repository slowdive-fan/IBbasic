...
namespace Raventhal.Droid
{
    public class SaveAndLoad_Android : ISaveAndLoad
    {
        ...
        public void CreateUserFolders()
        {
            ...
            string convertedFullPath = documents + "/Raventhal/saves";
        ...
        public void SaveText(string fullPath, string text)
        {
            ...
            string convertedFullPath = root + "/Raventhal" + ConvertFullPath(fullPath, "/");
        ...
        public string LoadStringFromUserFolder(string fullPath)
        {
            ...
            Stream stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets" + ConvertFullPath(fullPath, "."));
            if (stream != null)
            {
            ...
            string root = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            string filePath = root + "/Raventhal" + ConvertFullPath(fullPath, "/");
        ...
        public string LoadStringFromEitherFolder(string assetFolderpath, string userFolderpath)
        {
            ...
            string root = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            string filePath = root + "/Raventhal" + ConvertFullPath(userFolderpath, "/");
            if (File.Exists(filePath))
            {
                return File.ReadAllText(filePath);
            }
            //check in Assests folder last
            Assembly assembly = GetType().GetTypeInfo().Assembly;
            Stream stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets" + ConvertFullPath(assetFolderpath, "."));
            if (stream != null)
            {
                using (var reader = new System.IO.StreamReader(stream))
                {
        ...
        public string GetModuleFileString(string modFilename)
        {
            ...
            string root = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            string filePath = root + "/Raventhal/modules/" + modFolder + "/" + modFilename;
            if (File.Exists(filePath))
            {
                return File.ReadAllText(filePath);
            }
            ...
            Assembly assembly = GetType().GetTypeInfo().Assembly;
            Stream stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.modules." + modFilenameNoExtension + "." + modFilename);
            if (stream != null)
            {
                using (var reader = new System.IO.StreamReader(stream))
                {
        ...
        public SKBitmap LoadBitmap(string filename, IBbasic.Module mdl)
        {
            ...
            try
            {
                ...
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.modules." + mdl.moduleName + ".graphics." + filename + ".png");
                }
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.graphics." + filename);
                }
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.graphics." + filename + ".png");
                }
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.graphics." + filename + ".jpg");
                }
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.tiles." + filename);
                }
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.tiles." + filename + ".png");
                }
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.tiles." + filename + ".jpg");
                }
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.ui." + filename);
                }
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.ui." + filename + ".png");
                }
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.ui." + filename + ".jpg");
                }
                if (stream == null)
                {
                    stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.graphics.ui_missingtexture.png");
                }
            ...
            {
                Assembly assembly = GetType().GetTypeInfo().Assembly;
                Stream stream = assembly.GetManifestResourceStream("Raventhal.Droid.Assets.graphics.ui_missingtexture.png");
                SKManagedStream skStream = new SKManagedStream(stream);
                return SKBitmap.Decode(skStream);
            }
        }
        ...
        public List<string> GetAllFilesWithExtensionFromUserFolder(string folderpath, string extension)
        {
            ...
            string root = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            Java.IO.File directory = new Java.IO.File(root + "/Raventhal" + ConvertFullPath(folderpath, "/"));
            //directory.Mkdirs();
            if (directory.Exists())
            {
        ...
        public List<string> GetAllFilesWithExtensionFromBothFolders(string assetFolderpath, string userFolderpath, string extension)
        {
            ...
            //search in external folder
            string root = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            Java.IO.File directory = new Java.IO.File(root + "/Raventhal" + ConvertFullPath(userFolderpath, "/"));
            //directory.Mkdirs();
            if (directory.Exists())
            {
                foreach (Java.IO.File f in directory.ListFiles())
            ...
            foreach (var res in assembly.GetManifestResourceNames())
            {
                if ((res.Contains("Raventhal.Droid.Assets" + ConvertFullPath(userFolderpath, "."))) && (res.EndsWith(extension)))
                {
                    string[] split = res.Split('.');
                    list.Add(split[split.Length - 2]);
                }
            ...
            foreach (var res in assembly.GetManifestResourceNames())
            {
                if ((res.Contains("Raventhal.Droid.Assets" + ConvertFullPath(assetFolderpath, "."))) && (res.EndsWith(extension)))
                {
                    string[] split = res.Split('.');
                    list.Add(split[split.Length - 2]);
                }
        ...
        public List<string> GetAllModuleFiles(bool userOnly)
        {
            ...
            //search in external folder
            string root = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            Java.IO.File directory = new Java.IO.File(root + "/Raventhal/modules");
            if (directory.Exists())
            {
